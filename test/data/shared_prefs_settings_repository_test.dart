...
void main() {
  ...
  group('SharedPrefsSettingsRepository', () {
    ...
    test(
      'save and load roundtrip CustomSafFolder — uri and displayName',
      () async {
    ...
    test(
      'switching from custom back to system clears stored URI and displayName',
      () async {
        const uri = 'content://com.android.externalstorage/tree/primary';
        final repo = SharedPrefsSettingsRepository();
        await repo.save(
    ...
    test('createAuthorFolder persists as true', () async {
      final repo = SharedPrefsSettingsRepository();
      await repo.save(
        const AppSettings(target: SystemDownloads(), createAuthorFolder: true),
      );
      final loaded = await repo.load();
    ...
    test('createSeriesFolder persists as true', () async {
      final repo = SharedPrefsSettingsRepository();
      await repo.save(
        const AppSettings(target: SystemDownloads(), createSeriesFolder: true),
      );
      final loaded = await repo.load();

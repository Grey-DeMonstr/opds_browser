...
- **Develop natively on Windows.** The Android SDK and JDK are already installed on the Windows host; the repo already lives on `<project root>`. Native Windows development avoids the WSL2↔`/mnt/<drive>` filesystem performance penalty and gives smooth emulator/device access.

...
**Status: Step 1 already done (commit `a521232`).** Flutter project generated with defaults. Steps 2–4 verify the toolchain; Step 5 commits any lockfile update.

**Files already generated:** `pubspec.yaml`, `analysis_options.yaml`, `.gitignore`, `.metadata`, `lib/main.dart`, `android/` tree, `web/`, `windows/`
- Test: `test/widget_test.dart` may or may not be present (generated by `flutter create` by default)

- [x] **Step 1: Generate the project** *(already done — commit `a521232`)*

The project was generated with Flutter defaults (no `--org monster.greyde`, no `--platforms android`). As a result:
- `applicationId` and `namespace` are `com.example.opds_browser` — corrected in Task 6.
- `web/` and `windows/` directories are present — acceptable per spec §8 ("do not add beyond what flutter create generates").

No need to re-run flutter create.
...
Steps 2–4 are verification-only; `pubspec.lock` is already committed (included in `a521232`). If `flutter pub get` produced no lockfile changes, skip this step. If it did update `pubspec.lock`, commit it:

```bash
git add pubspec.lock
git commit -m "Update pubspec.lock after pub get"
```
...
Expected: `rm 'test/widget_test.dart'`. *(If the file does not exist, skip this step — nothing to remove.)*
...
Set `minSdkVersion 29` (parent spec §10.1), fix `applicationId`/`namespace` to `monster.greyde.opds_browser` (the project was generated with defaults in Task 1), and set the user-facing app label.

**Files:**
- Modify: `android/app/build.gradle.kts` (Kotlin DSL; recent Flutter) **or** `android/app/build.gradle` (Groovy; older Flutter)
- Modify: `android/app/src/main/AndroidManifest.xml`

- [ ] **Step 1: Set `minSdk` to 29 and fix `applicationId` + `namespace`**

In `android/app/build.gradle.kts` (Kotlin DSL), make these changes:

1. Inside `android { ... }`, change `namespace = "com.example.opds_browser"` to `namespace = "monster.greyde.opds_browser"`.
2. Inside `defaultConfig { ... }`, change `applicationId = "com.example.opds_browser"` to `applicationId = "monster.greyde.opds_browser"`.
3. Inside `defaultConfig { ... }`, change `minSdk = flutter.minSdkVersion` to `minSdk = 29`.

- [ ] **Step 2: Set the app display label**
...
- [ ] **Step 3: Verify the Dart toolchain is unaffected**
...
```bash
git add android/
git commit -m "Configure Android: minSdk 29, applicationId monster.greyde.opds_browser, OPDS Browser label"
```
